...
/**
 * Clase responsable de la trasnformación de parámetros de entrada incluidos en las peticiones de los servicios web publicados por @firma. 
 */
public final class CaibEsbTransformersFacade {

	/**
	 * Atributo para la escritura de trazas de ejecución.
	 */
	private static final Logger LOGGER = Logger.getLogger(CaibEsbTransformersFacade.class);

	/**
	 * Instancia de la clase. 
	 */
	private static CaibEsbTransformersFacade instance;

	/**
	 * Parámetros de configuración para la generación de peticiones.
	 */
	private Properties transformersProperties;

	/**
	 * Parámetros de configuración para el parseo de respuestas.
	 */
	private Properties parserParamsProp;

	private static final String TRANSFORMERS_TEMPLATES_ROOT_PATH = "transformersTemplates";
	
	private static final String TRANSFORMERS_TEMPLATES_REQUESTS_PATH = "xmlTemplates";
	
	private static final String TRANSFORMERS_TEMPLATES_RESPONSE_PATH = "parserTemplates";
	 ...
	 */
	public static CaibEsbTransformersFacade getInstance() {
		instance = new CaibEsbTransformersFacade();
		return instance;
	}
	...
	public Document getXmlRequestFileByRequestType(String serviceReq, String method, String type, String version) throws TransformersException {
		...
		LOGGER.debug(CaibEsbLanguage.getFormatResIntegra(ILogConstantKeys.TF_LOG004, new Object[ ] { serviceReq, method, type, version }));
		...
		try {
			...
			resourceName = transformersProperties.getProperty(templateName.toString());
			LOGGER.debug(CaibEsbLanguage.getFormatResIntegra(ILogConstantKeys.TF_LOG005, new Object[ ] { resourceName }));

			xmlTemplateResourcePath = "/" + TRANSFORMERS_TEMPLATES_ROOT_PATH + "/" + TRANSFORMERS_TEMPLATES_REQUESTS_PATH + "/" + resourceName;

			LOGGER.debug(CaibEsbLanguage.getFormatResIntegra(ILogConstantKeys.TF_LOG007, new Object[ ] { "/" + TRANSFORMERS_TEMPLATES_ROOT_PATH + "/" + TRANSFORMERS_TEMPLATES_REQUESTS_PATH }));
