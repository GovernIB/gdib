...
public final class UtilsFileSystem {
	...
	private static synchronized byte[ ] readFileFromFileSystem(String filePathToRead) {
		...
		if (filePathToRead != null) {
			try {
				...
				LOGGER.error(CaibEsbLanguage.getFormatResIntegra(ILogConstantKeys.UFS_LOG002, new Object[ ] { filePathToRead }), ioe);
				return null;
			} catch (Exception e) {
				LOGGER.error(CaibEsbLanguage.getFormatResIntegra(ILogConstantKeys.UFS_LOG003, new Object[ ] { filePathToRead }), e);
				return null;
			} finally {
				try {
					baos.close();
	...
	public static synchronized byte[ ] readFileFromFileSystemBase64Encoded(String filePathToRead) {
		if (filePathToRead != null) {
			try {
				...
				if (!CaibEsbBase64Coder.isBase64Encoded(content)) {
					return CaibEsbBase64Coder.encodeBase64(content);
				} else {
					return content;
				}
			} catch (Exception e) {
				LOGGER.error(CaibEsbLanguage.getFormatResIntegra(ILogConstantKeys.UFS_LOG004, new Object[ ] { filePathToRead }), e);
			}
		}
		return null;
	}
	...
	private static synchronized String getFilePath(String relativePath) {
		...
		if (url == null) {
			LOGGER.error(CaibEsbLanguage.getFormatResIntegra(ILogConstantKeys.UFS_LOG005, new Object[ ] { relativePath }));
			return null;
		}
		return url.getFile();
	}
	...
	public static synchronized String readFileBase64Encoded(String path, boolean isRelativePath) {
		...
		if (GenericUtils.assertStringValue(path)) {
			...
			throw new IllegalArgumentException(CaibEsbLanguage.getResIntegra(ILogConstantKeys.UFS_LOG001));
		}
		return fileContent;
	}
	...
	public static synchronized String getFileBase64Encoded(byte[ ] content) {
		...
		try {
			if (!CaibEsbBase64Coder.isBase64Encoded(content)) {
				fileContent = new String(CaibEsbBase64Coder.encodeBase64(content));
			} else {
				fileContent = new String(content);
			}
		} catch (Exception e) {
			LOGGER.error(CaibEsbLanguage.getResIntegra(ILogConstantKeys.UFS_LOG006), e);
		}
		return fileContent;
	...
	public static synchronized byte[ ] getArrayByteFileBase64Encoded(String path, boolean isRelativePath) {
		...
		if (GenericUtils.assertStringValue(path)) {
			fileContent = UtilsFileSystem.readFileFromFileSystemBase64Encoded(isRelativePath ? getFilePath(path) : path);
