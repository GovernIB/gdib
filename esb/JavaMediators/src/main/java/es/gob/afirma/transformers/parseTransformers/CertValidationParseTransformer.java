...
import es.caib.archivodigital.esb.services.mediators.afirma.transformers.CaibEsbTransformersProperties;
...
public class CertValidationParseTransformer implements IParseTransformer {
	...
	public final Object transform(String xmlResponse) throws TransformersException {
		...
		logger.debug(CaibEsbLanguage.getFormatResIntegra(ILogConstantKeys.CVPT_LOG002, new Object[ ] { xpathRespElement }));
		if (xpathRespElement == null) {
			throw new TransformersException(CaibEsbLanguage.getFormatResIntegra(ILogConstantKeys.CVPT_LOG001, new Object[ ] { TransformersConstants.RESP_ROOT_ELEMENT_CTE, request, messageVersion }));
		}
	...
	private Map<String, Object> readResponse(String xmlResponse, String xpathRespElement) throws TransformersException {
		...
		try {
			doc = CaibEsbUtilsXML.parseDocument(new StringReader(xmlResponse));
		} catch (Exception e) {
			throw new TransformersException(CaibEsbLanguage.getResIntegra(ILogConstantKeys.CVPT_LOG003), e);
		}
		...
		if (doc != null) {
			...
			if (!rootElement.getTagName().equals("mensajeSalida") && !rootElement.getTagName().equals("outputMessage")) {// el
				...
				// es
				// "mensajeSalida"
				throw new TransformersException(CaibEsbLanguage.getResIntegra(ILogConstantKeys.CVPT_LOG004));
			}

			res = new Hashtable<String, Object>();
			// Comprobamos si se ha producido algún error
			aux = readErrorResponse(CaibEsbUtilsXML.getElement(rootElement, ParseTransformerConstants.RESPONSE_ELEMENT_SP + "/" + ParseTransformerConstants.EXCEPTION_ELEMENT_SP));
			if (aux == null) {
				aux = readErrorResponse(CaibEsbUtilsXML.getElement(rootElement, ParseTransformerConstants.RESPONSE_ELEMENT + "/" + ParseTransformerConstants.EXCEPTION_ELEMENT));
			}
			if (aux == null) {
				// No se produjo ningún error, leemos el mensaje SOAP OK
				aux = readOKResponse(CaibEsbUtilsXML.getElement(rootElement, xpathRespElement));
				if (aux == null) {
					throw new TransformersException(CaibEsbLanguage.getResIntegra(ILogConstantKeys.CVPT_LOG005));
				}
	...
	private Map<String, Object> readErrorResponse(Element exceptionElement) {
		...
		if (exceptionElement != null) {
			...
			exceptionData = CaibEsbUtilsXML.searchChildElements(exceptionElement);
			for (int i = 0; i < exceptionData.size(); i++) {
				...
				if (aux.getNodeType() == Node.ELEMENT_NODE) {
					...
					// excepcionAsociada
					nodeValue = CaibEsbUtilsXML.getElementValue((Element) aux);
	...
	private Map<String, Object> readOKResponse(Element upperRespElement) {
		...
		if (upperRespElement != null) {
			...
			auxV = CaibEsbUtilsXML.searchChildElements(upperRespElement);
	...
	private Map<String, Object> parseInfoCert(Element infoCertElement) {
		...
		res = new Hashtable<String, Object>();
		auxV = CaibEsbUtilsXML.searchChildElements(infoCertElement);
		...
		for (int i = 0; i < auxV.size(); i++) {
			...
			if (auxN.getNodeType() == Node.ELEMENT_NODE && (auxN.getNodeName().equals("Campo") || "Field".equals(auxN.getNodeName()))) {
				fieldV = CaibEsbUtilsXML.searchChildElements((Element) auxN);
	...
	private void putFieldsValues(List<?> fieldV, Map<String, Object> result) {
		...
		if (fieldV.size() == 2) {
			...
			if (field.getNodeType() == Node.ELEMENT_NODE && (field.getNodeName().equals("idCampo") || "idField".equals(field.getNodeName()))) {
				fieldId = CaibEsbUtilsXML.getElementValue((Element) field);
				field = (Node) fieldV.get(1);
				fieldValue = CaibEsbUtilsXML.getElementValue((Element) field);
	...
	private Map<String, Object> parseInfoVal(Element infoValElement) {
		...
		if (infoValElement != null) {
			// Obtenemos la información de respuesta
			auxV = CaibEsbUtilsXML.searchChildElements(infoValElement);
	...
	private Map<String, Object> parseInfoValRec(List<?> elementChilds, int index) {
		...
		if (elementChilds != null && index < elementChilds.size()) {
			...
			if (auxN.getNodeType() == Node.ELEMENT_NODE) {
				auxV = CaibEsbUtilsXML.searchChildElements((Element) auxN);

				if (auxV != null && auxV.size() > 0) {
					res.put(auxN.getNodeName(), parseInfoValRec(auxV, 0));
				} else {
					nodeValue = CaibEsbUtilsXML.getElementValue((Element) auxN);
	...
	private String getXpathRespElement() {
		...
		properties = CaibEsbTransformersProperties.getMethodParseTransformersProperties(request, method, messageVersion);
