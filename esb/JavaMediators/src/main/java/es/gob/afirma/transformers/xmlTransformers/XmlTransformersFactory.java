...
public final class XmlTransformersFactory {
	...
	public static Class<?> getXmlTransformer(String serviceReq, String method, String type, String version) throws TransformersException {
		...
		try {
			if (serviceReq == null || method == null || version == null) {
				throw new TransformersException(CaibEsbLanguage.getFormatResIntegra(ILogConstantKeys.XTF_LOG001, new Object[ ] { serviceReq, method, version }));
			}
			transformerClass = getTransformerClassName(serviceReq, method, type, version);
			if (transformerClass == null) {
				throw new TransformersException(CaibEsbLanguage.getFormatResIntegra(ILogConstantKeys.XTF_LOG002, new Object[ ] { serviceReq, method, version }));
			}
			...
			if (!found) {
				res = null;
				throw new TransformersException(CaibEsbLanguage.getFormatResIntegra(ILogConstantKeys.XTF_LOG003, new Object[ ] { transformerClass, IXmlTransformer.class.getName() }));
			}

			logger.debug(CaibEsbLanguage.getFormatResIntegra(ILogConstantKeys.XTF_LOG004, new Object[ ] { transformerClass }));
	...
	private static String getTransformerClassName(String serviceReq, String method, String type, String version) {
		...
		if (type.equals(TransformersConstants.REQUEST_CTE)) {
			properties = CaibEsbTransformersProperties.getMethodRequestTransformersProperties(serviceReq, method, version);
		} else if (type.equals(TransformersConstants.RESPONSE_CTE)) {
			properties = CaibEsbTransformersProperties.getMethodResponseTransformersProperties(serviceReq, method, version);
		}
