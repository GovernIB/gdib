...
public class AnyOutParameterParseTransformer implements IParseTransformer {
	...
	public final Object transform(String xmlResponse) throws TransformersException {
		...
		logger.debug(CaibEsbLanguage.getFormatResIntegra(ILogConstantKeys.AOPPT_LOG002, new Object[ ] { xpathRespElement }));
		if (xpathRespElement == null) {
			throw new TransformersException(CaibEsbLanguage.getFormatResIntegra(ILogConstantKeys.AOPPT_LOG001, new Object[ ] { TransformersConstants.RESP_ROOT_ELEMENT_CTE, request, messageVersion }));
		}
	...
	private Map<String, Object> readResponse(String xmlResponse, String xpathRespElement) throws TransformersException {
		...
		try {
			...
			throw new TransformersException(CaibEsbLanguage.getResIntegra(ILogConstantKeys.AOPPT_LOG003), e);
		}
		...
		if (doc != null) {
			...
			if (!rootElement.getTagName().equals("mensajeSalida") && !rootElement.getTagName().equals("outputMessage")) {// el
				...
				// es
				// "mensajeSalida"
				throw new TransformersException(CaibEsbLanguage.getResIntegra(ILogConstantKeys.AOPPT_LOG004));
			}

			res = new Hashtable<String, Object>();
			// Comprobamos si se ha producido algún error (en servicios en
			// español e inglés)
			aux = readErrorResponse(CaibEsbUtilsXML.getElement(rootElement, "respuesta/Excepcion"));
			if (aux == null) {
				aux = readErrorResponse(CaibEsbUtilsXML.getElement(rootElement, "response/Exception"));
			}
			if (aux == null) {
				...
				if (aux == null) {
					throw new TransformersException(CaibEsbLanguage.getResIntegra(ILogConstantKeys.AOPPT_LOG005));
				}
