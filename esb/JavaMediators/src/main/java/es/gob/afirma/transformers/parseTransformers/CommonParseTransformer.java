...
public class CommonParseTransformer implements IParseTransformer {
	...
	public final Object transform(String xmlResponse) throws TransformersException {
		...
		logger.debug(CaibEsbLanguage.getFormatResIntegra(ILogConstantKeys.CPT_LOG002, new Object[ ] { xpathRespElement }));
		if (xpathRespElement == null) {
			throw new TransformersException(CaibEsbLanguage.getFormatResIntegra(ILogConstantKeys.CPT_LOG001, new Object[ ] { TransformersConstants.RESP_ROOT_ELEMENT_CTE, request, messageVersion }));
		}
	...
	private Map<String, Object> readResponse(String xmlResponse, String xpathRespElement) throws TransformersException {
		...
		try {
			...
			throw new TransformersException(CaibEsbLanguage.getResIntegra(ILogConstantKeys.CPT_LOG003), e);
		}
		...
		if (doc != null) {
			...
			if (!rootElement.getTagName().equals("mensajeSalida") && !rootElement.getTagName().equals("outputMessage")) {// el
				...
				// es
				// "mensajeSalida"
				throw new TransformersException(CaibEsbLanguage.getResIntegra(ILogConstantKeys.CPT_LOG004));
			}

			res = new Hashtable<String, Object>();
			// Comprobamos si se ha producido algún error.
			Element errorElem = CaibEsbUtilsXML.getElement(rootElement, ParseTransformerConstants.RESPONSE_ELEMENT_SP + "/" + ParseTransformerConstants.EXCEPTION_ELEMENT_SP);
			if (errorElem == null) {
				errorElem = CaibEsbUtilsXML.getElement(rootElement, ParseTransformerConstants.RESPONSE_ELEMENT + "/" + ParseTransformerConstants.EXCEPTION_ELEMENT);
			}
			if (errorElem != null) {
			...
			if (aux == null) {
				...
				if (aux == null) {
					throw new TransformersException(CaibEsbLanguage.getResIntegra(ILogConstantKeys.CPT_LOG005));
				}
