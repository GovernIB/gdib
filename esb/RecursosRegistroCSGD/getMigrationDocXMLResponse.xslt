			...
			<xsl:if test="$resultadoCorrecto">
				<csgd:resParam>
					...
					<csgd:binaryContents>
						...
						<xsl:if test="fn:boolean(//wsm:result/ws:content/ws:data/text())">
						...
						<xsl:if test="fn:boolean(//wsm:result/ws:content/ws:byteSize/text())">
							<csgd:byteSize><xsl:value-of select="//wsm:result/ws:content/ws:byteSize/text()"/></csgd:byteSize>
						</xsl:if>
					</csgd:binaryContents>
