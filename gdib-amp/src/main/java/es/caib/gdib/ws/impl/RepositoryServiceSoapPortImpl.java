...
public class RepositoryServiceSoapPortImpl extends SpringBeanAutowiringSupport implements RepositoryServiceSoapPort{
	...
	/**
	 * Modos de enlazar documentos:
	 *    - Copy : Se copia de un nodo a otro nodo
    ...
    /**
     * Localizacion de ficheros temporales
     * */
	...
	/**
     * Limite de resultados obtenidos en la bÃºsqueda
     * */
    ...
    /**
	 * @param minCustodyAdvancedSignatureForm the minCustodyAdvancedSignatureForm to set
	 */
	...
	/**
	 * Genera una version en el nodo.
	 * @param nodeRef NodeRef del nodo que se quiere avanzar una version
	 *
	 * */
    ...
    /**
     * El metadato eni:id de un nodo siempre se va a calcular salvo si la operacion del ESB
     * es "dispatchDocument" y si una la property "gdib.createNode.dispatchDocument.eni_id.noReplace" es true.
     *
     * En ese caso, el eni:id va a venir informado porque es un documento que viene del registro
     * y entonces no se va a calcular
     *
     *
     ...
     * @param node Nodo que se acaba de crear y que habria que aplicar dicha plantilla.
    ...
    private void getTemplate(NodeRef node) throws GdibException{
    	// busco en las plantillas de alfresco si la serie documental de este expediente tiene una plantilla de expediente
    ...
    public NodeRef _internal_createNode(NodeRef parentRef, QName name, QName type, Map<QName,Serializable> props, Content content, DataHandler sign)throws GdibException{
	 ...
	 * Realiza la operacion de createNode sin comprobaciones previas y con todos los elementos:
	 *  - Firma
	 *  - Contenido
	 *  - Metadatos
	 *
	 ...
	 * @param  aspects Lista<QName> de aspectos que tendra el nuevo nodo.
	 ...
	 * */
	private NodeRef _internal_createNode(NodeRef parentRef, QName name, QName type, Map<QName,Serializable> props, List<QName> aspects, Content content, DataHandler sign, String esbOperation)throws GdibException{
	 ...
	 /**
	 * Desacoplamiento de createNode.
	 * Realiza la operacion de createNode sin comprobaciones previas con los elementos:
	 *  - Contenido
	 *  - Metadatos
	 *
	 * @param  parentRef NodeRef del nodo padre donde se va a generar el nuevo nodo.
	 ...
	 * @throws GdibException Si los aspectos o propiedades son incorrectas
	 * */
     private NodeRef _internal_createNode(NodeRef parentRef, QName name, QName type, Map<QName,Serializable> props, Content content)throws GdibException{
     ...
     /**
 	 * Desacoplamiento de createNode.
 	 * Realiza la operacion de createNode sin comprobaciones previas con sus metadatos
 	 *
 	 * @param  parentRef NodeRef del nodo padre donde se va a generar el nuevo nodo.
 	 * @param  name Nombre del nuevo nodo.
 	 * @param  type QName del tipo del nuevo nodo
    ...
    /**
   	 ...
   	 * Realiza la operacion de getNode sin comprobaciones previas.
   	 *
   	 ...
   	 * @return Node Objeto Nodo con la informacion que alfresco tiene en el repositorio.
    ...
    /**
   	 ...
   	 * Realiza la operacion de copia de un nodo sin comprobaciones previas.
   	 *
   	 ...
   	 * @throws GdibException Si no se tienen permisos para crear el nodo en la nueva localizacion.
   	 * @throws GdibException Si no se tienen permisos para leer el nodo source.
   	 * @throws GdibException Si el nodo copiado ya existia previamente
    ...
    private NodeRef _internal_copy(NodeRef target, NodeRef source) throws GdibException{
        ...
        try{
            ...
            copy = fileFolderService.copy(source, target, expedienteName + "_" + new SimpleDateFormat("yyyyMMddHHmm").format(new Date())).getNodeRef();
    ...
    /**
   	 * Desacoplamiento de linkNode.
   	 * Realiza la operacion de enlace de un nodo sin comprobaciones previas.
   	 * La operacion de enlazar dispone un enlace padre-hijo entre nodos.
   	 *
   	 ...
   	 * @throws GdibException Si no se tienen permisos para enlazar el nodo en la nueva localizacion.
   	 *
   	 * */
    ...
    /**
   	 ...
   	 * Realiza la operacion de movimiento de un nodo sin comprobaciones previas.
   	 *
   	 * @param  nodeRef del nodo que se pretende mover
   	 * @param  newParentNodeRef NodeRef del nodo padre donde se va a mover el nodo.
   	 * @throws GdibException Si no se tienen permisos para mover el nodo en la nueva localizacion.
   	 * @throws GdibException Si el nodo que se quiere mover esta bloqueado por otro usuario.
   	 * */
     ...
     * Realiza la asignacion/desasignacion de permisos a unas autoridades en concreto para unos nodos de alfresco sin comprobaciones previas.
     *
     ...
     * */
    private void _internal_authorizeNode(List<NodeRef> nodeRefs, List<String> authorities, String permission) throws GdibException {
    ...
    /**
     * Desacoplamiento de authorizeNode
     * Realiza la asignacion/desasignacion de permisos a una sola autoridad en concreto para un nodo de alfresco sin comprobaciones previas.
     *
    ...
    /**
     * Desacoplamiento de authorizeNode
     * Realiza la asignacion/desasignacion de una lista de permisos a una autoridad en concreto para un nodo de alfresco sin comprobaciones previas.
     *
     * @param nodeRef Nodo que se pretende modificar sus ACLs
     * @param authority Autoridad que va a modificar los permisos del nodo.
    ...
    /**
     * Genera y firma el indice electronico del expediente.
     *
	...
	private DataHandler _internal_foliate(NodeRef nodeRef, String indexType) throws GdibException {
		...
		try {
			...
			// 2 Firma del indice electronico
			...
			case AdministrativeProcessingIndexSignerFactory.ENI_INDEX_V10:
				...
				//Se especifica como referencia de firma el elemento raiz expediente
				...
				break;
			default:
				throw new GdibException("El tipo de indice " + indexType + " no es soportado.");
			}
	...
	/**
	 * Comprobacion de la firma de un documento.
	 *
    ...
    private void checkDocumentSignature(Node node) throws GdibException{
    	...
    	LOGGER.debug("Se inicia la validacion de la firma electronica del documento " + nodeIdValue);
    	...
    	if(signatureTypeProp == null){
    		throw new GdibException("La propiedad o metadato " + EniModelUtilsInterface.PROP_TIPO_FIRMA + " del documento " +
    				nodeIdValue + " no ha sido establecida.");
    	}

    	EniSignatureType eniSignatureType = EniSignatureType.valueOf(signatureTypeProp);
    	if(eniSignatureType == null){
    		throw new GdibException("La propiedad o metadato " + EniModelUtilsInterface.PROP_TIPO_FIRMA + " del documento " +
    				nodeIdValue + " tiene un valor no admitido: " + signatureTypeProp + ".");
    	}

        try{
        	// si es un documento, es version definitiva y si la firma es realizada mediante certificado electronico
        	if(utils.isType(node.getType(), ConstantUtils.TYPE_DOCUMENTO_QNAME)
        		...
        		if(!EniSignatureType.TF05.equals(eniSignatureType) &&
        			...
        			// Los tipos de firmas TF02,TF03 y TF04 requieren informar el contenido del documento para validar la firma electronica
        		...
        		LOGGER.debug("Preparando invocacion a plataforma @firma (ValidarFirma)...");
        		...
        		LOGGER.debug("Parseando resultado de validacion de la firma electronica del documento " + node.getId() + ".");
        		if(result.getValidationStatus() == ValidationStatus.CORRECTO){
        			LOGGER.debug("Resultado de validacion de la firma electronica del documento " + (node.getId() == null? "nuevo documento": node.getId()) + " correcto.");
        			//Si la firma es correcta, se verifica que el formato avanzado es igual o superior al manimo exigido
        			...
        			LOGGER.debug("Se verifica que la firma presenta un formato avanzado igual o superior al manimo exigido...");
        			...
        			for(int i=0; !found && i<custodyAdvancedSignatureFormats.length;i++){
        				if(custodyAdvancedSignatureFormats[i].toUpperCase().startsWith(eniSignatureType.getName().toUpperCase())){
        					...
        					LOGGER.debug("Formato manimo exigido configurado para el tipo de firma " + eniSignatureType.getName() + ": " + custodyAdvancedSignatureFormats[i] + ".");
        					found = Boolean.TRUE;
        				}
        			}

        			if(!found || SignatureFormat.UNRECOGNIZED.equals(minCustodySignatureFormat) || eniSignatureProfile == null){
        				throw new GdibException("Sistema mal cofigurado, debe establecerse un formato de firma electronica avanzado manimo para el tipo de firma " +
        						eniSignatureType.getName() + ".");
        			}

        			//Si el formato de firma es inferior, se evoluciona al minimo exigido
        			if(minCustodySignatureFormat.isMoreAdvancedSignatureFormat(signatureFormat)){
        				LOGGER.debug("Formato de firma inferior al minimo exigido para custodia, se procede a evolucionar la firma al formato: " +
        				...
        				LOGGER.debug("Modificando firma electronica del documento " + nodeIdValue + "....");
        			...
        			//Si no es asi, se modifica el metadato
        			if(!signatureProfileNodeProp.equals(eniSignatureProfile.getName())){
        				...
        				for(int i = 0;!propFound && i < node.getProperties().size();i++){
        					if(node.getProperties().get(i).getQname().endsWith(EniModelUtilsInterface.PROP_PERFIL_FIRMA)){
        						node.getProperties().get(i).setValue(eniSignatureProfile.getName());
        						LOGGER.debug("Metadato " + EniModelUtilsInterface.PROP_PERFIL_FIRMA + " del documento " + nodeIdValue +
                						" encontrado y modificado. Nuevo valor: " + eniSignatureProfile.getName() + ".");
        						propFound = Boolean.TRUE;
        					}
        				}
        				if ( !propFound ){        					
        					nodeService.setProperty( utils.idToNodeRef(node.getId()), EniModelUtilsInterface.PROP_PERFIL_FIRMA_QNAME, eniSignatureProfile.getName());
        				}
    				}

        			LOGGER.debug("Validacion firma electronica del documento " + nodeIdValue + " finalizada.");
        		} else if(result.getValidationStatus() == ValidationStatus.NO_CORRECTO){
        			LOGGER.debug("Resultado de validacion de la firma electronica del documento " + node.getId() + " incorrecto.");
        			...
        			LOGGER.debug("Resultado de validacion de la firma electronica del documento " + node.getId() + " no determinado.");
    		...
    		LOGGER.debug("Finalizada la validacion de la firma electronica del documento " + node.getId());
    	}
    }
    ...
    /**
     * Compara y aÃ±ade/borra/modifica las propiedades de un nodo para la operacion de modifyNode
     *
     ...
     *
     * */
    private void modifyNodeProperties(Node node, List<Property> originalProperties, String esbOperation, boolean isFinalNode) throws GdibException {
        ...
        String nameOriginal = originalProperties.get(originalProperties.indexOf(new Property(ContentModel.PROP_NAME))).getValue();
        ...
        String newNameProp = "";
        if ( newNameIndex != -1 ) newNameProp = node.getProperties().get(newNameIndex).getValue();
        ...
        if ( "".equals(newNameProp)) {
        	...
        	if ( newNameProp.equals(nameOriginal)){ // El nombre en las props es el original
        		 ...
        		 * punto solo caben dos opciones, ambos son iguales (no habria que hacer nada) o son diferentes,
        		 * con lo que habria que lanzar una excepcion porque se esta intentando modificar dos veces el nombre
        		 * */
        ...
        // Si el nodo es final, comprobamos que no se cambia un conjunto de propiedades que no se pueden modificar
        if(isFinalNode)
    ...
    /**
     * Compara y aÃ±ade/borra/modifica los aspectos de un nodo para la operacion de modifyNode
     *
     ...
     */
    private void modifyNodeAspects(NodeRef updateNode, List<String> updateAspects, List<String> originalAspects) throws GdibException {
    ...
    /**
     * Modifica el contenido de un nodo para la operacion modifyNode.
     *
    ...
    private void modifyContentNode(NodeRef node,  Content updateContent) throws GdibException{
        ...
        try{
            ...
            if(!utils.compareDataHandlers(updateContent.getData(),originalContent.getData()) &&
            		!isFinalNode){
                // validacion de no realizar esta operacion con documentos finales y si cambia el contenido
           		throw exUtils.setContentException(node.getId(), new IOException("No se puede modificar el contenido de un documento definitivo."));
            }else{
            	utils.setDataHandler(node, ContentModel.PROP_CONTENT, updateContent.getData(), updateContent.getMimetype());
            }
            */
    ...
    /***
     * Modifica la firma de un nodo para la operacion de modifyNode
     *
    ...
    private void modifySignNode(NodeRef node,  DataHandler updateSign) throws GdibException{
        ...
        try{
        	...
        	/*
            if(!utils.compareDataHandlers(updateSign,originalSign)
            		&& isFinalNode){
            	// validacion de no realizar esta operacion con documentos finales y si cambia la firma
            	throw exUtils.setContentException(node.getId(), new IOException("No se puede modificar la firma en documentos finales"));
            }else{
            	utils.setDataHandler(node, ConstantUtils.PROP_FIRMA_QNAME, updateSign, MimetypeMap.MIMETYPE_BINARY);
            }*/
        }catch(IOException exception){
    ...
    /**
     * Generacion del export de un expediente sin comprobaciones previas.
     *
...
//		    	NodeRef exportExpedient = _internal_copy(tempNode, nodeRef);
		    	// tengo que copiar el expediente cambiando el nombre y todos los documentos aÃ±adiendo al nombre la _fecha
...
//		    	NodeRef exportFolder = fileFolderService.searchSimple(exportExpedient, ConstantUtils.EXPEDIENT_EXPORT_FOLDER_NAME);
		    	// si no existe la carpeta es porque es la primera exportacion y no tenemos que borrarla
//		    	if(exportFolder!=null)	fileFolderService.delete(exportFolder);
		    	NodeRef ie_node = null;
		    	try {
			 		// Folio el expediente generando el XML de indice electronico ENI firmado
				    ...
				    ie_node = _internal_createNode(exportExpedient, utils.createNameQName(exchangeIndexNodeName), ConstantUtils.TYPE_FILE_INDEX_QNAME, props);
		    	...
		    	// muevo el expediente de exportacion a la carpeta "export" dentro del expediente
		    	NodeRef exportFolder = fileFolderService.searchSimple(nodeRef, ConstantUtils.EXPEDIENT_EXPORT_FOLDER_NAME);
		    	// si es la primera vez que se exporta esta carpeta no existira dentro del expediente, y tenemos que crearla
		    	if(exportFolder==null)
		    		exportFolder = fileFolderService.create(nodeRef, ConstantUtils.EXPEDIENT_EXPORT_FOLDER_NAME, ConstantUtils.TYPE_FOLDER).getNodeRef();
    ...
    private NodeRef copyExpedientExport(NodeRef temp, NodeRef exp, String dateName, Boolean isFileRoot) throws GdibException{
    	// genero el nuevo expediente
    	NodeRef newExp = null;
    	String name = (String) nodeService.getProperty(exp, ConstantUtils.PROP_NAME);
    	newExp = copyService.copyAndRename(exp, temp, ContentModel.ASSOC_CONTAINS, utils.createNameQName(name+dateName), false);
    	...
    	for (ChildAssociationRef child:childs){
			...
			if (fileFolderService.getFileInfo(son).isFolder()){
		    	...
		    	NodeRef newSon = copyService.copyAndRename(son, newExp, ContentModel.ASSOC_CONTAINS, utils.createNameQName(nameSon+dateName), false);
    ...
    /**
     * Funcion que bloquea un nodo sin comprobaciones previas
     *
    ...
    public String _createNode(Node node, String parentId, GdibHeader gdibHeader) throws GdibException {
		...
		if (!repositoryDisableCheck.booleanValue()) {
			if (!utils.contains(node.getAspects(), ConstantUtils.ASPECT_BORRADOR_QNAME)) {
				...
				LOGGER.debug("comprobacion integridad del nodo.");
				utils.checkNodeIntegrity(node);
				verifySubtypeDoc(node);				
			}
		}
		LOGGER.debug("Preparacion para la llamada al servicio");
        // preparar datos
        ...
        LOGGER.debug("Se llama al servicio de creacion de nodos");
     ...
     * Se realizaron las comprobaciones necesarias: Permisos, formato de metadatos, tipo, etc...
     * Ademas se realizan dos comprobaciones adicionales dependiendo del flag repositoryDisableCheck esta activo o no:
     *  1Âº Que el nodo creado estÃ© dentro del DM.
     *  2Âº Que tenga una serie documental valida.
     *  El formato de QName siempre que venga especificado como un String podra ser tanto formato extendido {uri}prop
     *   o reducido prefix:prop .
     *
     *  Ademas los Ids de nodos podran venir especificados tanto en nodeRef como en paths relativos tomando siempre como primer nodo
     ...
     *  @param gdibHeader cabecera de auditoria y configuracion.
    ...
    /**
     * Comprueba que la serie documental de un nodo, subexpediente o documento, sea la misma que la de su expediente padre
     * @param node Nodo a verificar
     * @param parentRef Nodo padre inmediato
     * @throws GdibException si el nodo y el expediente padre no pertenecen a la misma serie documental.
     */
    private void checkDocClassification(Node node, NodeRef parentRef) throws GdibException{

    	// Check para eni:documento y eni:expediente (subexpediente), de la serie documental. Nodo y padre expediente deben tener el mismo codigo de clasificacion
        ...
        if (utils.isType(node.getType(), ConstantUtils.TYPE_DOCUMENTO_QNAME) ||
        	...
        	if (!StringUtils.isEmpty(nodeCodClasif) && !nodeCodClasif.equals(parentCodCasif)){
        		throw exUtils.checkMetadataValueException(ConstantUtils.PROP_COD_CLASIFICACION_QNAME.toString(), nodeCodClasif);
        	}
        }
    }
    ...
    /**
     * Servicio que modifica la informacion de un nodo de alfresco.
     ...
     * Impide la modificacion de la firma o contenido de un documento en estado final.
     * La forma en que se modifica un nodo es la siguiente:
     * 1Âº Se especifica al servicio el nodo con los metadatos o propiedades que se quieran. Estos podran ser iguales o diferentes de los originales.
     *    Los metadatos/aspectos que sean iguales no se hace nada con ellos. Aquellos que sean diferentes se cambiaran. Los que no aparezcan
     *    tampoco se modificaran. Y para eliminar propiedades o metadatos sera necesario especificarlo mediante el caracter "-" en su QName.
     *    Por ejemplo: -eni:id eliminaria la propiedad ID.
    ...
    public void modifyNode(Node node, GdibHeader gdibHeader) throws GdibException {
	        ...
	        // Esto NO se puede hacer -> this.getNode(nodeRef.getId(), false, false, gdibHeader);
	        ...
	        if (utils.contains(original.getAspects(), ConstantUtils.ASPECT_BORRADOR_QNAME)
		        ...
		        //momento en el que se verifica la firma electronica del mismo, y se evoluciona si no presenta el formato minimo exigido.
	        	...
	        	String newperfil = utils.getProperty(newNode.getProperties(), EniModelUtilsInterface.ENI_MODEL_PREFIX + EniModelUtilsInterface.PROP_PERFIL_FIRMA);
	        	LOGGER.info("Perfil de firma despues de comprobar firma: " + newperfil);
	        	
	        	if ( !perfil.equals(newperfil)){
	        		//El perfil de firma ha cambiado.
	        		String signatureTypeProp = utils.getProperty(newNode.getProperties(), EniModelUtilsInterface.ENI_MODEL_PREFIX + EniModelUtilsInterface.PROP_TIPO_FIRMA);
	        		EniSignatureType eniSignatureType = EniSignatureType.valueOf(signatureTypeProp);
	        		if(!EniSignatureType.TF01.equals(eniSignatureType) &&
	        				!EniSignatureType.TF04.equals(eniSignatureType)){
	        ...
	        // para crear versiones menores, se puede pasar "VersionType.MAJOR" para una version mayor
     ...
     *   @param withContent Flag que indica si se requiere la recuperacion del contenido
     *   @param withSigh Flag que indica si se requiere la recuperacion de la firma.
     *   @param gdibHeader Cabecera de auditoria y configuracion.
     *   @return Objeto con la informacion del nodo.
    ...
    /**
     * Servicio que mueve la localizacion de un nodo padre a otro diferente.
     * @param nodeId Identificador de nodo a mover
     * @param newParent Identificador del nuevo padre
     * @param gdibHeader Cabecera de auditoria y configuracion
     * @throws GdibException Si no se tienen permisos para realizar la operacion (permisos de escritura en ambos nodos).
     *
     *
     * */
    @Override
    public void moveNode(String nodeId, String newParent, GdibHeader gdibHeader) throws GdibException {
	        ...
	        //Se verifica que la nueva localizacion pertence a la misma serie documental
    ...
    /**
     * Servicio de bÃºsqueda de nodos en los indices.
     * El nÃºmero de resultados esta condicionado por la variable gdib.repository.search.limit
     * La funcion recibe la pagina de la bÃºsqueda que funciona de la siguiente manera:
     * 	* La variable limite establece el numero de registros por paginas de la busqueda. Si la
     ...
     *
     * El numero total de paginas y nodos encontrados va en la peticion, si por la razon que sea se
     * solicita un numero mayor que lo que hay realmente no devolvera resultados, por ejemplo:
     *  * Una busqueda devuelve 1000 resultados con una configuracion de limite de 100.
     *  * El usuario pide la pagina 10 ( 10*100 + 1 = 1001, devolveria los resultados entre 1001 y 1100)
     *    la bÃºsqueda no devolveria ningÃºn resultado.
     *
     ...
     *
     *  Enlace LINK: Genera un enlace padre hijo nuevo en el nodo. El nodo conservaroa los enlaces anteriores padre-hijo. Esto significa que dos
     *  			carpetas podrian ver exactamente el mismo nodo, con su contenido y propiedades calcadas, y no ocuaparia espacio extra en repositorio.
     *  Enlace COPY: Genera un NUEVO NODO en el repositorio hijo del nodo padre suministrado. Esto implica que los dos nodos mantendrian diferentes
     *  			metadatos y contenidos, pero al ser imposible la modificacion del contenido de un nodo si es definitivo, en la practica
     ...
     *
     *  Resumen:
     *   * Enlace Link comparten metadatos y contenido
     *   * Enlace Copy comparten contenido y diferentes metadatos.
     *
     * @param parentId Id del nuevo padre del nodo
     * @param nodeId Id del nodo que se quiere enlazar
     * @param linkMode Tipo de enlace
     * @param gdibHeader cabecera de auditoria y configuracion
     * @throws GdibException Si no se tienen permisos para realizar la operacion.
     *
     * */
    @Override
    public String linkNode(String parentId, String nodeId, String linkMode, GdibHeader gdibHeader) throws GdibException {
	    	long initMill = System.currentTimeMillis();
    ...
    private String _internal_linkNode(NodeRef parentRef, NodeRef nodeRef, String linkMode) throws GdibException {
        ...
        if ( linkMode.equals(LINK_COPY)){
            ...
            if(!hasDraftAspect){
	        	...
	        	if(!StringUtils.isEmpty(parentCodCasif) && !parentCodCasif.equals(serieDocNodo)){
	        		throw exUtils.checkMetadataValueException(ConstantUtils.PROP_COD_CLASIFICACION_QNAME.toString(), serieDocNodo);
	        	}
            }
	...
	/**
     * Servicio que genera un indice electronico.
     * @param nodeId Identificador del expediente a foliar.
     * @param GdibHeader cabecera de auditoria y configuracion.
     * @throws DataHandler con el contenido del indice.
     * @throws GdibException si ocurre algÃºn error en la generacion del foliado o no se tienen los permisos adecuados para realizarla.
     *
     * */
    @Override
    public DataHandler foliateNode(String nodeId, GdibHeader gdibHeader) throws GdibException {
	    	...
	    	LOGGER.info("Se inicia  la generacion del indice interno del expediente " + nodeId);
	    	NodeRef nodeRef = utils.checkNodeId(nodeId);
		...
		String fileStatus = (String) nodeService.getProperty(nodeRef, ConstantUtils.PROP_ESTADO_EXP_QNAME);
	     ...
	     // miro si el expediente esta en estado abierto
	        if(ConstantUtils.ESTADO_EXP_E01.equals(fileStatus)){
	        	//El expediente se encuentra en DM, se genera el indice interno
     ...
     * El servicio debe generar un subexpediente del mismo con la informacion actual del expediente y su indice obviando el resto de
     * subexpedientes de exportacion.
     *
     * @param nodeId Identificador del nodo a exportar.
     * @param gdibHeader Cabecera de seguridad y configuracion.
     * @return Identificador del subexpediente generado.
    ...
    public DataHandler exportNode(String nodeId, GdibHeader gdibHeader) throws GdibException {
	        ...
	        if(ConstantUtils.ESTADO_EXP_E01.equals(fileStatus)){
	        	...
	        	//Recupero los hijos y si es de tipo indice de intercambio lo devuelvo.
     ...
     * @param gdibHeader Cabecera de auditoria y configuracion.
     * @return List<NodeVersion> Lista de informacion de versiones.
	...
	/**
	 * Servicio que asigna un permiso a una lista de nodos para unas autoridades dadas.
	 *
	 ...
	 * */
    @Override
    public void authorizeNode(List<String> nodeIds, List<String> authorities, String permission, GdibHeader gdibHeader) throws GdibException {
	        ...
	        if ( nodeIds.size() > 1 ){
	        	LOGGER.info("Permisos ["+nodeIds.get(0)+".."+nodeIds.get(nodeIds.size()-1)+"] cambiados en "+(System.currentTimeMillis()-initMill)+"ms.");
	        }else if (nodeIds.size() == 1) {
	        	LOGGER.info("Permisos de "+nodeIds.get(0)+" cambiados en "+(System.currentTimeMillis()-initMill)+"ms.");
	        }else{
    ...
    /**
     * Servicio que elimina todos los permisos que tuvieran las autoridades en una lista de nodos.
	 *
     ...
     *  - Si la autoridad no existe => Excepcion.
     *  - Si el nodo esta bloqueado => Excepcion.
	 * @param nodeIds Lista de identificadores de nodo
	 * @param authorities Lista de autoridades
	 * @param gdibHeader Cabecera de seguridad y configuracion.
     *
     * */
    @Override
	public void removeAuthority(List<String> nodeIds, List<String> authorities, GdibHeader gdibHeader) throws GdibException{
			...
			if ( nodeIds.size() > 1 ){
	        	...
	        	LOGGER.info("Permisos de "+nodeIds.get(0)+" eliminados en "+(System.currentTimeMillis()-initMill)+"ms.");
	        }else{
	...
	/**
	 * Servicio que recupera la informacion de migracion de un nodo migrado y transformado.
	 *
	...
	public MigrationInfo getMigrationInfo(String nodeId, GdibHeader gdibHeader) throws GdibException {
			...
			ret.setValcertSign(
					utils.getContent(nodeRef, ConstantUtils.PROP_FIRMAVALCERT_QNAME ).getData()
			);
			ret.setZipContent(
					utils.getContent(nodeRef, ConstantUtils.PROP_ZIPMIGRACION_QNAME ).getData()
			);
			LOGGER.info("Info de migracion de "+nodeId+" recuperada en "+(System.currentTimeMillis()-initMill)+"ms.");
			return ret;
	}
	...
	public String getCSV(GdibHeader gdibHeader) throws GdibException {
    	...
    	try{
			...
			try {
				...
				throw exUtils.csvError(e);
			/*} catch (GdibTransactionException e) {
				throw new GdibException(e);*/
			}
		}catch(GdibException e){
			//rollback();
			throw e;
	...
	/**
	 * Apertura de un expediente tras haberse cerrado. El sistema genera un nuevo expediente con la informacion del anterior.
	 *
	...
	public String openFile(String nodeId, GdibHeader gdibHeader) throws GdibException {
			...
			// Se modifica el estado de tramitacion del expediente, asignandole el valor â€œCerradoâ€.
			properties.put(ConstantUtils.PROP_ESTADO_EXP_QNAME, ConstantUtils.ESTADO_EXP_E01);
			// Se modifica el estado de archivo del expediente, asignandole el valor â€œpreingresoâ€.
			...
			LOGGER.info("Se procede a establecer propiedades de archivado (interoperables) a los documentos del expediente. Propiedades: " + properties);
	...
	/**
	 * Restaura los enlazes a expedientes relacionados, sus uuid estan guardados en la propiedad "gdib:exp_enlazado"
	 * si hay subexpedientes dentro del expediente, tengo que restaurar sus expedientes enlazados tambien
	 *
	 * @param expedientRef
	 * @throws GdibException
	 */
	...
	private void restoreLinkedExpedient(NodeRef expedientRef) throws GdibException{
		...
		List<String> uuids = (List<String>) nodeService.getProperty(expedientRef, ConstantUtils.PROP_EXP_ENLAZADO_QNAME);
	 ...
	 * @param gdibHeader Cabecera de configuracion y auditoria.
	 *
	 * */
	@Override
	public void closeFile(String nodeId, GdibHeader gdibHeader) throws GdibException {
			...
			LOGGER.debug("Obteniendo la la clasificacion documental del expediente " + nodeId);
			...
			LOGGER.debug("Clasificacion documental del expediente " + nodeId + ": " +
	...
	private NodeRef _internal_closeFile(NodeRef expedientRef, Date closeDate) throws GdibException, ContentIOException, IOException{
		...
		DataHandler exchangeIndexHandler = _internal_foliate(expedientRef, AdministrativeProcessingIndexSignerFactory.ENI_INDEX_V10);
	    NodeRef exchangeIndexNodeRef = this._internal_createNode(expedientRef, utils.createNameQName(exchangeIndexNodeName), ConstantUtils.TYPE_FILE_INDEX_QNAME, indexsProps);
		...
		// Se efectia la transferencia a la fase semi- activa del expediente.
	...
	/**
	 * Establece las propiedades de archivado a un expediente y su contenido cuando este es cerrado.
	 ...
	 private void setFileContentArchivedMetadataCollection(NodeRef nodeRef, Map<QName, Serializable> properties, boolean isFileRoot) throws GdibException {
         ...
         try{
                 ...
                 if(utils.isType(nodeService.getType(nodeRef), ConstantUtils.TYPE_EXPEDIENTE_QNAME)){
                         ...
                         docProp.put(ConstantUtils.PROP_FASE_ARCHIVO_QNAME, properties.get(ConstantUtils.PROP_FASE_ARCHIVO_QNAME));
                         // Se modifica la fecha fin de expediente para el documento
                         docProp.put(ConstantUtils.PROP_FECHA_FIN_EXP_QNAME, properties.get(ConstantUtils.PROP_FECHA_FIN_EXP_QNAME));
	...
	/**
	 * Voy recorriendo todos los nodos del expediente, y cuando encuentro un nodo que es expediente enlazado,
	 * guardo en la propiedad "gdib:exp_enlazado" el uuid de dicho expediente, para luego poder restaurar
	 * el enlaze de dicho expediente
	 *
	 * @param expedientRef
	...
	private void safeLinkedExpedient(NodeRef expedientRef) throws InvalidNodeRefException, GdibException{
        ...
        while ( !pilaNodos.isEmpty() ){
        	...
        	// compruebo si es un expediente o agregacion, si es un documento no se hace nada y se pasa al siguiente nodo
        	if(utils.isType(nodeService.getType(node), ConstantUtils.TYPE_EXPEDIENTE_QNAME)){
        		if(!nodeService.getPrimaryParent(node).getParentRef().equals(expedientRef)){
        			// es un expediente enlazado porque su primary parent no es el expediente que estoy tratando
        			uuidEnlazado.add(node.getId());
        		}else{
        			// es un subexpediente, por lo que tengo que hacer la misma operacion, por eso la
        			// llamada recursiva
        			safeLinkedExpedient(node);
        		}
        	}else if(utils.isType(nodeService.getType(node), ConstantUtils.TYPE_AGREGACION_DOC_QNAME)){
        		// si el nodo es una carpeta de agregacion, obtengo todos los hijos de dicha carpeta y los apilo
	...
	public SearchResults _internal_searchNode(String luceneQuery, int pagina) throws GdibException{
        ...
        // esos resultados serian incorrectos... y no aparecerian en la bÃºsqueda, pero no se puede capar por permisos
        ...
        long endMill = System.currentTimeMillis();
        LOGGER.info("searchNode (Busqueda: "+(busMill-initMill)+"ms, Composicion nodos: "+(endMill-busMill)+"ms)");
        return res;
	}


	/**
	 * Recupera el indice de intercambio de un nodo en el RM
	 * @throws GdibException
	 *
	 * */
...
/*
	private void createTransaction(GdibHeader gdibHeader) throws GdibException {
		...
		try {
			LOGGER.info("Transaccion recuperada. Estado: "+ String.valueOf(usrTrx.getStatus()));
			usrTrx.begin();
			LOGGER.info("Transaccion iniciada.  Estado: "+ String.valueOf(usrTrx.getStatus()));
	...
	private void rollback() throws GdibException {
		...
		try {
			 if ( usrTrx.getStatus() == Status.STATUS_ACTIVE){
				 usrTrx.rollback();
			 }
		} catch (IllegalStateException e) {
			LOGGER.error(e.getMessage(), e);
